...
export default {
  ...
  created() {
    cellId += this.shapes.length;
  },
  computed: {
    shapes() {
      return this.init.shapes;
    },
    length() {
      return this.shapes.length;
    },
    shape() {
      return this.shapes[this.idx % this.length];
    },
    style() {
      return {
        left: this.x * this.cellSize + "px",
        bottom: this.y * this.cellSize + "px"
      };
    },
    cells() {
      return this.shape.map(([x, y], idx) => {
        return {
          ...
          data: { class: this.init.class },
          id: this.fromCellId + idx
        };
      });
    }
  },
  methods: {
    ...
    predictMove(x, y) {
      return this.shape.map(([_x, _y]) => [_x + this.x + x, _y + this.y + y]);
    },
    rotate(x = 0) {
      this.x += x;
      this.idx++;
